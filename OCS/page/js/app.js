...
(function() {
  ...
  const ROUTES = {};
  let FILE_TO_ROUTE = {};
  let SITE = { name:'Open-code-Studio', titleSuffix:'Open-code-Studio 文档', meta:'Open-code-Studio · 文档站点', docDir:'../docs' };
  let NAV = [];
  let SIDEBAR_LINKS = [];
  const DEFAULT_ROUTE = '/';
  ...
  const ICONS = {
    ...
    projects: '<rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/>',
    join: '<circle cx="12" cy="10" r="3"/><path d="M12 21v-8M12 13v-3"/><path d="M9 16l-3 3"/><path d="M15 16l3 3"/>',
  ...
  async function init() {
    ...
    window.OWD = {
      config: SITE, routes: ROUTES, nav: NAV, cache: pageCache,
      ...
      async searchAll(query) {
        await Promise.all(Object.keys(ROUTES).map(r => this.getPage(r)));
