/* ============================================
   openweb.kit-MD3 Docs - Main Application
   ============================================ */
...
(function() {
  ...
  // === Configuration ===
  const ROUTES = {
    '/':            { file: 'README.md',      title: '概览' },
    '/quick-start': { file: 'quick-start.md',  title: '快速开始' },
    '/css':         { file: 'css.md',          title: 'CSS 架构' },
    '/js':          { file: 'js.md',           title: 'JS 配置' },
    '/customize':   { file: 'customize.md',    title: '自定义主题' },
    '/info':        { file: 'info.md',         title: '关于' },
  };

  // Build filename → route reverse mapping
  const FILE_TO_ROUTE = {};
  ...
  async function loadDocument(route) {
    ...
    document.title = `${config.title} - openweb.kit-MD3`;
    ...
    try {
      ...
      renderer.heading = function({ text, depth }) {
        ...
        return `<h${depth} id="${anchorId}"><a class="heading-anchor" href="#${anchorId}" aria-hidden="true">#</a>${text}</h${depth}>`;
      };
      ...
      docMeta.innerHTML = `<span>openweb.kit-MD3 · 文档站点框架</span>`;

    } catch (err) {
      console.error('Failed to load document:', err);
      docBody.innerHTML = `<div class="loading-state" style="gap:12px"><svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="var(--md-sys-color-error)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg><p>文档加载失败</p><p style="font-size:14px;color:var(--md-sys-color-on-surface-variant)">${err.message}</p><button onclick="location.reload()" style="margin-top:12px;padding:8px 24px;border-radius:9999px;border:none;background:var(--md-sys-color-primary);color:var(--md-sys-color-on-primary);cursor:pointer;font-size:14px">重新加载</button></div>`;
    }
  }
  ...
  // === Intercept .md / .txt links → hash route (auto-fallback) ===
